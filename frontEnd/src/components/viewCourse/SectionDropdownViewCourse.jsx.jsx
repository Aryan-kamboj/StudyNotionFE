  ...
  return (
    <div>
      ...
      {Show?section?.lectures?.map((lecture,i)=>{
        return <div onClick={clickHandler} className='text-richblack-50 h-[2rem] flex items-center space-x-2 px-6' key={i}>{Watched?<MdCheckBox/>:<MdCheckBoxOutlineBlank/>}<span>{lecture.lectureTitle}</span></div>}
        ):""}
    </div>
  )
}
