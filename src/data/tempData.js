export const testUserProfile = {
    fname:"Aryan",
    lname:"Kamboj",
    email:"aryankambozz@gmail.com",
    phoneNo:{
        number:"9149345908",
        countryCode:"+91-India",
    }, 
    profilePhoto:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    gender:"Male",
    bio:"no Bio",
    DOB:"Dob string",
}
export const myCoursesData = [
{
    title:"CourseName1",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    duration:913543,
    createdAt:1699461335664,
    published:true,
    description:"the undoubtable source. Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of de Finibus Bonorum et Malorum (The Extremes of Good and Evil) by Cicero, written in 45 BC. This book is a treatise on the theory of ethics, very",
    price:999
},
{
    title:"CourseName2",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    duration:64802,
    createdAt:1699461234567,
    published:false,
    description:" If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the ",
    price:1000
},
{
    title:"CourseName3",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
    duration:43,
    createdAt:1699460987654,
    published:true,
    description:"chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested. Sections 1.10.32 and 1.10.33 from de Finibus Bonorum et Malorum",
    price:99
},
{
    title:"CourseName4",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    duration:59123,
    createdAt:1699462222222,
    published:false,
    description:" If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the ",
    price:9
},
{
    title:"CourseName5",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/balloons.jpg",
    duration:99943,
    createdAt:1699467645327,
    published:true,
    description:"the undoubtable source. Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of de Finibus Bonorum et Malorum (The Extremes of Good and Evil) by Cicero, written in 45 BC. This book is a treatise on the theory of ethics, very",
    price:1
},
{
    title:"CourseName6",
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    duration:89123,
    createdAt:1699461234543,
    published:false,
    description:"chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested. Sections 1.10.32 and 1.10.33 from de Finibus Bonorum et Malorum",
    price:8293
},
]
export const testCartData = [{
    title:"Course 1 title",
    price:100,
    courseId:"courseId1",
    rating:3.7,
    reviewCount:200,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    instructorName:"Aryan kamboj",
},
{
    title:"Course 2 title",
    price:101,
    courseId:"courseId2",
    rating:3.2,
    reviewCount:201,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    instructorName:"Aryan kamboj2",
},
{
    title:"Course 3 title",
    price:103,
    courseId:"courseId3",
    rating:3.9,
    reviewCount:203,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/balloons.jpg",
    instructorName:"Aryan kamboj3",
},
{
    title:"Course 4 title",
    price:104,
    courseId:"courseId4",
    rating:4,
    reviewCount:204,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj4",
},
{
    title:"Course 5 title",
    price:105,
    courseId:"courseId5",
    rating:5,
    reviewCount:205,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj5",
},
{
    title:"Course 5 title",
    price:105,
    courseId:"courseId5",
    rating:5,
    reviewCount:205,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj5",
},
{
    title:"Course 4 title",
    price:104,
    courseId:"courseId4",
    rating:4,
    reviewCount:204,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj4",
},
{
    title:"Course 5 title",
    price:105,
    courseId:"courseId5",
    rating:5,
    reviewCount:205,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj5",
},
{
    title:"Course 5 title",
    price:105,
    courseId:"courseId5",
    rating:5,
    reviewCount:205,
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    instructorName:"Aryan kamboj5",
}
]
export const EnrolledCoursesData = [
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 1",
            courseDesc:"short desc",
            courseId:"course1",
            timeSpent:5000,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 2",
            courseDesc:"short desc 2",
            courseId:"course2",
            timeSpent:50,
            progress:24,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
            courseName:"Course 3",
            courseDesc:"short desc 3",
            courseId:"course3",
            timeSpent:30240,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 1",
            courseDesc:"short desc",
            courseId:"course1",
            timeSpent:5000,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 2",
            courseDesc:"short desc 2",
            courseId:"course2",
            timeSpent:50,
            progress:24,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
            courseName:"Course 3",
            courseDesc:"short desc 3",
            courseId:"course3",
            timeSpent:30240,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 1",
            courseDesc:"short desc",
            courseId:"course1",
            timeSpent:5000,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 2",
            courseDesc:"short desc 2",
            courseId:"course2",
            timeSpent:50,
            progress:24,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
            courseName:"Course 3",
            courseDesc:"short desc 3",
            courseId:"course3",
            timeSpent:30240,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 1",
            courseDesc:"short desc",
            courseId:"course1",
            timeSpent:5000,
            progress:50,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
            courseName:"Course 2",
            courseDesc:"short desc 2",
            courseId:"course2",
            timeSpent:50,
            progress:24,
        },
        {
            courseThumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
            courseName:"Course 3",
            courseDesc:"short desc 3",
            courseId:"course3",
            timeSpent:30240,
            progress:50,
        },
    ]
export const catagories = ["catagory 1","catagory2","catagory3"];
export const mostPopularCourses = [{
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    title:"Course 1 title",
    price:100,
    reviewCount:200,
    rating:3.7,
    id:"courseId1",
},
{
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/balloons.jpg",
    title:"Course 2 title",
    price:200,
    reviewCount:300,
    rating:4.5,
    id:"courseId2",
},
{
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384068/samples/look-up.jpg",
    title:"Course 3 title",
    price:300,
    reviewCount:400,
    rating:5,
    id:"courseId3",
}]
export const courseDetails ={
    thumbnail:"https://res.cloudinary.com/studynotion/image/upload/v1688384067/samples/smile.jpg",
    title:"Course 1 title",
    description:"short sa description",
    price:100,
    reviewCount:200,
    rating:3.7,
    id:"courseId1",
    enrolledNo:32,
    createdAt:1698652872554,
    language:"English",
    instructorName:"Aryan kamboj",
    instructorImg:"https://res.cloudinary.com/studynotion/image/upload/v1688384066/samples/shoe.jpg",
    benifits:["benefit1","benefit2"],
    requirements:["Requirement1","requirement2","jfklasdfjaklfjasklfjdkslfjsaljjh kjkljkljlkjkjkljkljlkjlkjhjhhjhgjk hjkkjkljkjiuooilkjhjhhjhgjkhjkkjkljkjiuooi"],
    courseContent:[
    {
        section:"section1",
        lectures:[{
            title:"lecture1",
            length:25,
            description:"this is the Description of lecture1"
        }]
    },
    {
        section:"section2",
        lectures:[
            {
                title:"lecture2",
                length:21,
                description:"this is the Description of lecture2"
            },
            {
                title:"lecture3",
                length:23,
                description:"this is the Description of lecture3"
            }]
    },
    {
        section:"section3",
        lectures:[
        {
            title:"lecture4",
            length:21,
            description:"this is the Description of lecture2"
        },
        {
            title:"lecture5",
            length:23,
            description:"this is the Description of lecture3"
        },
        {
            title:"lecture6",
            length:6,
            description:"this is the Description of lecture6"
        }]
    },
    {
        section:"section4",
        lectures:[{
            title:"lecture7",
            length:25,
            description:"this is the Description of lecture1"
        }]
    },
    ]
}